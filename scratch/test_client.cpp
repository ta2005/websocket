...
Task<void> run_async_client(async::EventLoop& loop) {
    ...
    // For this test, we assume the user has a connect function, or we can just mock it.
    // We'll mock the fd with 0 just to see if the code compiles.
    int dummy_fd = 0; 
    async::TcpSocket socket(dummy_fd, loop);
    
    auto client_res = co_await Client<async::TcpSocket>::create(std::move(socket), "echo.websocket.events", "/");
    if (!client_res) {
        std::cout << "[ASYNC] Failed to create client: " << static_cast<int>(client_res.error()) << "\n";
        co_return;
    }
    ...
    if (!send_res) {
        std::cout << "[ASYNC] Failed to send: " << static_cast<int>(send_res.error()) << "\n";
        co_return;
    }
    
    auto chunk = co_await client.read_chunk();
    if (!chunk) {
        std::cout << "[ASYNC] Failed to read chunk: " << static_cast<int>(chunk.error()) << "\n";
        co_return;
    }
...
void run_sync_client() {
    ...
    auto socket_res = TcpSocket::connect("echo.websocket.events", "80");
    if (!socket_res) {
        std::cout << "[SYNC] Connection failed\n";
        return;
    }
    
    auto client_res = Client<TcpSocket>::create(std::move(*socket_res), "echo.websocket.events", "/");
    if (!client_res) {
        std::cout << "[SYNC] Failed to create client: " << static_cast<int>(client_res.error()) << "\n";
        return;
    }
    ...
    if (!send_res) {
        std::cout << "[SYNC] Failed to send: " << static_cast<int>(send_res.error()) << "\n";
        return;
    }
    
    auto chunk = client.read_chunk();
    if (!chunk) {
        std::cout << "[SYNC] Failed to read chunk: " << static_cast<int>(chunk.error()) << "\n";
        return;
    }
