...
TcpSocket::send(std::span<const uint8_t> meta_data,
    ...
    io[0].iov_base = const_cast<void *>(static_cast<const void *>(meta_data.data()));
    ...
    while (total_sent < total_size) {
        // BUG FIX: The 3rd argument is the number of buffers (iovcnt), not the byte size!
        ...
        if (total_sent < total_size) {
            ...
            if (total_sent >= meta_data.size() && current_index == 0) {
                // The metadata buffer is fully sent, move onto the payload buffer
                current_index = 1;
                iovcnt = 1;
...
std::expected<size_t, std::string_view>
TcpSocket::send(const std::span<const uint8_t> meta_data, std::string_view payload) const{
